...
export default function ChatSidebar({
  ...
  const roomIcon = (room: any) => {
    ...
    if (room.type === "group") return <span className="text-sm font-black text-white">#</span>;
    return <span className="text-sm font-black text-white">D</span>;
  };
  ...
  return (
    <div className="w-64 shrink-0 bg-[#030303] border-r border-[#1a1a1a] flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-[#1a1a1a] bg-[#020202]">
        <div className="flex items-center gap-3">
          <div className="flex flex-col items-center leading-none shrink-0">
            ...
            <span className="text-[8px] font-black tracking-[0.2em] text-white/40 uppercase mt-0.5">BTC</span>
          </div>
          <div>
            <p className="font-black text-sm tracking-[0.25em] uppercase text-white leading-none">VBC</p>
            <p className="text-[10px] text-neutral-600 tracking-widest uppercase mt-0.5">Volegram Bitcoin Chat</p>
          </div>
        </div>
      </div>
      ...
      <button onClick={() => nav(`/u/${user.username}`)}
        ...
        <div className="w-10 h-10 rounded-full border border-white/20 overflow-hidden bg-[#111] shrink-0">
          {user.avatarUrl
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-white truncate group-hover:text-white/70 transition-colors">@{user.username}</p>
        ...
        <User size={13} className="text-neutral-700 group-hover:text-white/60"/>
      </button>
        ...
        {[["chats", "Chats", <MessageCircle size={13}/>], ["swap", "Swap", <Zap size={13}/>]].map(([id, label, icon]: any) => (
          <button key={id} onClick={() => { setTab(id); if (id === "swap") onToggleSwap(); }}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold uppercase tracking-widest border-b-2 transition-colors
              ${(tab === id || (id === "swap" && showSwap)) ? "border-white text-white" : "border-transparent text-neutral-600 hover:text-white"}`}>
            {icon}{label}
          </button>
        ))}
      </div>
          ...
          {newMode === null && (
            <div className="flex gap-1">
              <button onClick={() => setNew("dm")}
                className="flex-1 flex items-center gap-1 text-xs text-neutral-600 hover:text-white font-bold uppercase transition-colors py-1">
                <Plus size={11}/> DM
              </button>
            ...
            <form onSubmit={startDM} className="space-y-1">
              <div className="flex gap-1">
                <input value={dmTarget} onChange={e => setDmT(e.target.value)} placeholder="@username" autoFocus
                  className="flex-1 bg-[#0a0a0a] border border-[#2a2a2a] text-white text-xs px-2 py-1.5 outline-none focus:border-white font-mono"/>
                <button type="submit" className="bg-white text-black text-xs px-2 font-black">GO</button>
            ...
            <form onSubmit={startIncognito} className="space-y-1">
              <div className="flex gap-1">
                <input value={icoName} onChange={e => setIcoN(e.target.value)} placeholder="Room name (opt)" autoFocus
                  className="flex-1 bg-[#0a0a0a] border border-purple-900/40 text-white text-xs px-2 py-1.5 outline-none focus:border-purple-500 font-mono"/>
              ...
              {invite && (
                <div className="bg-purple-900/20 border border-purple-900/40 px-2 py-1.5 rounded">
                  ...
                  <div className="flex items-center gap-1">
                    <code className="text-xs text-white font-mono flex-1">{invite}</code>
                    <button type="button" onClick={() => navigator.clipboard.writeText(invite)}
                      className="text-xs text-purple-400 hover:text-white px-1.5 py-0.5 border border-purple-800">COPY</button>
                  </div>
                </div>
              )}
              {err && <p className="text-xs text-red-500">{err}</p>}
            ...
            <form onSubmit={joinRoom} className="space-y-1">
              <div className="flex gap-1">
                <input value={joinCode} onChange={e => setJoin(e.target.value)} placeholder="Invite code" autoFocus
                  className="flex-1 bg-[#0a0a0a] border border-[#2a2a2a] text-white text-xs px-2 py-1.5 outline-none focus:border-white font-mono"/>
                <button type="submit" className="bg-white text-black text-xs px-2 font-black">JOIN</button>
                <button type="button" onClick={closeNew} className="text-neutral-700 px-1 hover:text-white text-xs">✕</button>
              </div>
              {err && <p className="text-xs text-red-500">{err}</p>}
            </form>
            ...
            <button key={room.id} onClick={() => onSelectRoom(room)}
              className={`w-full px-4 py-3.5 text-left flex items-center gap-3 border-b border-[#0d0d0d] transition-colors
                ${activeRoomId === room.id
                  ? room.isIncognito ? "bg-purple-900/10 border-l-2 border-l-purple-600" : "bg-white/5 border-l-2 border-l-white"
                  : "hover:bg-white/3"}`}>
      ...
      <div className="border-t border-[#1a1a1a] px-4 py-3 space-y-2">
        {user.isAdmin && (
          <button onClick={() => nav("/admin")}
            className="w-full flex items-center gap-2 text-xs text-white hover:text-neutral-400 font-bold uppercase tracking-widest transition-colors">
