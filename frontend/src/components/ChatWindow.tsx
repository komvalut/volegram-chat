...
import { Zap, Paperclip, Mic, Send, Image, Flame, X } from "lucide-react";
...
export default function ChatWindow({ room, user }: { room: any; user: any }) {
  ...
  useEffect(() => {
    setMessages([]);
    api.messages(room.id).then(msgs => {
    ...
    const handler = (msg: any) => {
      if (msg.type === "message" && msg.message.roomId === room.id) {
        if (!msg.message.isDeleted) setMessages(prev => [...prev, msg.message]);
      }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-3 border-b border-[#1a1a1a] bg-[#030303] flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-[#FF6A00]/20 border border-[#FF6A00]/30 flex items-center justify-center text-xs font-black text-[#FF6A00] shrink-0">
          {room.type === "group" ? "#" : "D"}
        </div>
        <div className="flex-1">
          <p className="text-sm font-black text-white">{room.name ?? (room.type === "dm" ? "Direct Message" : "Group")}</p>
          <p className="text-[10px] text-neutral-600">{room.type} · encrypted</p>
        </div>
        {burnSecs > 0 && (
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        {messages.length === 0 && (
          <div className="text-center py-10 text-[10px] text-neutral-700">No messages yet. Say hello ⚡</div>
        )}
        {messages.map(m => (
        ...
        {typing && (
          <div className="flex items-center gap-2 text-[10px] text-neutral-600 mb-2 ml-9">
      ...
      {showBurn && (
        <div className="px-4 py-2 border-t border-[#1a1a1a] bg-[#030303] flex items-center gap-2">
          ...
          <span className="text-[10px] text-neutral-500 uppercase tracking-wider mr-1">Burn timer:</span>
          {BURN_OPTIONS.map(o => (
            <button key={o.value} onClick={() => { setBurnSecs(o.value); setShowBurn(false); }}
              className={`text-[10px] px-2.5 py-1 border font-bold transition-colors
          ...
          <button onClick={() => setShowBurn(false)} className="ml-auto text-neutral-700 hover:text-white">
            <X size={12}/>
          </button>
        </div>
      )}
      ...
      <div className="px-4 py-3 border-t border-[#1a1a1a] bg-[#030303]">
        <div className="flex items-end gap-2">
          <div className="flex gap-1 pb-1">
            ...
            <button onClick={() => setShowLN(true)} className="p-2 text-neutral-600 hover:text-[#FF6A00] transition-colors">
              <Zap size={15}/>
            </button>
            ...
            <button onClick={() => setShowBurn(p => !p)}
              className={`p-2 transition-colors ${burnSecs > 0 ? "text-orange-400" : "text-neutral-600 hover:text-orange-400"}`}
          ...
          <button onClick={sendText} disabled={!text.trim()}
            className={`p-2.5 text-black hover:bg-[#e55500] disabled:opacity-40 transition-colors
              ${burnSecs > 0 ? "bg-orange-600" : "bg-[#FF6A00]"}`}>
            <Send size={15}/>
          </button>
        </div>
      </div>
