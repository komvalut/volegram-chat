...
router.post("/listings", auth, async (req, res) => {
  ...
  const { title, description, priceSats, currency, paymentMethod } = req.body;
  if (!title || !priceSats || priceSats < 1)
    return res.status(400).json({ error: "title and priceSats required" });
  try {
    const rows = await db.execute(sql`
      INSERT INTO vbc_listings (seller_id, title, description, price_sats, currency, payment_method)
      VALUES (${sellerId}, ${title}, ${description ?? ""}, ${priceSats}, ${currency ?? "BTC"}, ${paymentMethod ?? "Lightning"})
      RETURNING *
    `);
...
router.post("/listings/:id/contact", auth, async (req, res) => {
  ...
  try {
    ...
    const existing = (await db.execute(sql`
      ...
      JOIN chat_members ma ON ma.room_id = r.id AND ma.user_id = ${buyerId}
      JOIN chat_members mb ON mb.room_id = r.id AND mb.user_id = ${sellerId}
      WHERE r.type = 'dm'
      LIMIT 1
    `)).rows[0];
    ...
    if (existing) {
      ...
      await db.execute(sql`INSERT INTO chat_members (room_id, user_id) VALUES (${roomId}, ${buyerId}), (${roomId}, ${sellerId})`);
    }

    // Send intro message
    const [seller] = (await db.execute(sql`SELECT username FROM chat_users WHERE id = ${sellerId}`)).rows;
    const intro = `Hi! I'm interested in your listing: "${(listing as any).title}" — ⚡${Number((listing as any).price_sats).toLocaleString()} sats (${(listing as any).currency})`;
    const [msg] = (await db.execute(sql`
      INSERT INTO chat_messages (room_id, sender_id, type, content)
      VALUES (${roomId}, ${buyerId}, 'text', ${intro})
      RETURNING *
    `)).rows;
