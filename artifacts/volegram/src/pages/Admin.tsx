      ...
      <div className="flex-1 overflow-y-auto p-6">
        {tab === "users" && (
          <>
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full max-w-md bg-neutral-50 border border-neutral-200 text-black text-xs px-3 py-2 mb-4 outline-none focus:border-black font-mono"/>
            <div className="space-y-1">
              {filtered.map(u => (
                <div key={u.id} className={`flex items-center gap-4 px-4 py-3 border ${u.isBlocked ? "border-red-900/30 bg-red-900/5" : "border-neutral-200 bg-[#050505]"} hover:border-neutral-300`}>
        ...
        {tab === "reports" && (
          <div className="space-y-2">
            {reports.length === 0 && <p className="text-xs text-neutral-500">No reports yet.</p>}
            {reports.map(r => (
              <div key={r.id} className={`px-4 py-3 border ${r.resolved ? "border-neutral-200 opacity-50" : "border-yellow-900/40 bg-yellow-900/5"}`}>
                <div className="flex items-center gap-3 mb-1">
                  <AlertTriangle size={12} className="text-yellow-500"/>
                  <span className="text-xs text-black">Report #{r.id}</span>
                  <span className="text-xs text-neutral-600">{new Date(r.createdAt).toLocaleString()}</span>
                  {r.resolved && <span className="text-xs text-green-600 ml-auto">Resolved</span>}
                </div>
                ...
                {!r.resolved && (
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => resolve(r.id)}
                      className="text-xs border border-green-800 text-green-400 px-2 py-1 hover:bg-green-900/20">
                      RESOLVE
                    </button>
                    <button onClick={() => block(r.targetId, false)}
                      className="text-xs border border-red-800 text-red-400 px-2 py-1 hover:bg-red-900/20">
                      BLOCK USER
                    </button>
                  </div>
                )}
