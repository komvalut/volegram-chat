  ...
  return (
    <div className="h-full bg-white overflow-y-auto">
      <div className="max-w-lg mx-auto px-4 py-6">
        <button onClick={() => nav(-1)} className="flex items-center gap-2 text-neutral-600 hover:text-black text-xs mb-6 transition-colors">
          <ArrowLeft size={14}/> Back
        </button>

        {/* Profile Card */}
        <div className="border border-neutral-200 bg-[#050505] p-6 mb-4">
          <div className="flex items-start gap-5">
            ...
            <div className="relative shrink-0">
              ...
              {isMe && editing && (
                <label className="absolute bottom-0 right-0 cursor-pointer bg-[white] rounded-full p-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <button onClick={copyLN} className="text-neutral-500 hover:text-[white] transition-colors">
                  {copied ? <Check size={11} className="text-green-400"/> : <Copy size={11}/>}
                </button>
              </div>
              <p className="text-xs text-neutral-500 mt-1">Member since {new Date(profile.createdAt).toLocaleDateString()}</p>
            </div>

            {isMe && !editing && (
              <button onClick={() => setEditing(true)}
                className="text-xs border border-neutral-300 text-neutral-500 hover:text-black hover:border-[white] px-3 py-1.5 transition-colors">
                EDIT
              </button>
            )}
          </div>
          ...
          {profile.bio && !editing && (
            <p className="mt-4 text-sm text-neutral-400 leading-relaxed border-t border-neutral-200 pt-4">{profile.bio}</p>
          )}
        </div>
        ...
        {!isMe && currentUser && (
          <div className="flex gap-2">
            <button onClick={() => setShRp(!showReport)}
              className="flex items-center gap-1.5 text-xs border border-neutral-200 text-neutral-600 hover:text-red-400 hover:border-red-900 px-3 py-2 transition-colors">
