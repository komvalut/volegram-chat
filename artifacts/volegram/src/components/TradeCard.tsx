...
export default function TradeCard({
  ...
  return (
    <>
      <div className="border border-black/20 bg-gradient-to-b from-[#0d0800] to-[#060400] p-4 max-w-[320px] w-full">
        {/* Header */}
        ...
        <div className="bg-black/40 px-3 py-2.5 mb-3 border border-black/5">
          <div className="flex items-center gap-2 text-sm font-extrabold">
            {isFiat ? (
              <>
                ...
                <span className="text-black">⚡ {trade.sats.toLocaleString()} sats</span>
              </>
            ) : (
              <>
                <span className="text-black">⚡ {trade.sats.toLocaleString()} sats</span>
                <ArrowRight size={11} className="text-neutral-600"/>
                <span className="text-black">{trade.assetAmount} {trade.asset}</span>
              </>
            )}
          </div>
          ...
          {/* Fee breakdown */}
          {(trade.feeSats ?? 0) > 0 && (
            <div className="mt-2 pt-2 border-t border-black/5 grid grid-cols-3 text-xs text-neutral-500">
        ...
        {/* Status */}
        <div className={`flex items-center gap-1.5 text-xs font-bold mb-1 ${st.color}`}>
          {trade.status === "released" ? <CheckCircle size={11}/> :
           trade.status === "disputed" ? <AlertTriangle size={11}/> :
           <Clock size={11}/>}
          {st.label}
        </div>
        {st.desc && <p className="text-xs text-neutral-600 mb-3">{st.desc}</p>}
        ...
        {!isFiat && (<>
          ...
          {isBuyer && trade.status === "pending" && trade.invoicePr && (
            <div className="space-y-2">
              <div className="bg-black/60 border border-black/10 px-2 py-1.5">
                <p className="text-xs text-neutral-600 mb-1">Lightning Escrow Invoice:</p>
          ...
          {!isBuyer && trade.buyerAddress && (
            <div className="bg-black/60 border border-purple-900/40 px-3 py-2 mb-2">
              <p className="text-xs text-purple-400 font-bold mb-1">Send {trade.assetAmount} {trade.asset} to:</p>
              <div className="flex items-center gap-2">
                <code className="text-xs text-black font-mono break-all flex-1">{trade.buyerAddress}</code>
                <button onClick={() => navigator.clipboard.writeText(trade.buyerAddress ?? "")} className="shrink-0 text-neutral-500 hover:text-black">
                  <Copy size={10}/>
                </button>
              </div>
            </div>
          ...
          {isBuyer && trade.status === "address_shared" && (
            <div className="space-y-1.5">
              ...
              <button onClick={dispute} className="w-full text-xs text-neutral-600 hover:text-red-400 py-1">
        ...
        {isFiat && (<>
          ...
          {isBuyer && trade.status === "pending" && (
            <div className="space-y-2">
              <div className="bg-blue-900/10 border border-blue-900/40 px-3 py-2">
                <p className="text-xs text-blue-300 font-bold mb-1">1. Send payment</p>
                <p className="text-xs text-neutral-500">{trade.assetAmount}</p>
                <p className="text-xs text-neutral-600 mt-1">
                  Contact @{trade.seller.username} for their payment details, then upload your Revolut / bank screenshot below.
                </p>
              </div>
              <p className="text-xs text-black font-bold mt-1">2. Upload payment proof:</p>
              ...
              <p className="text-xs text-neutral-500 text-center">Revolut, bank app, PayPal screenshot</p>
            </div>
          )}
          ...
          {isBuyer && trade.status === "proof_submitted" && (
            <div className="bg-blue-900/10 border border-blue-900/40 px-3 py-2">
              <p className="text-xs text-blue-300 font-bold">✓ Proof submitted</p>
              <p className="text-xs text-neutral-600 mt-1">Waiting for @{trade.seller.username} to confirm and release BTC.</p>
            </div>
          )}
          ...
          {isSeller && trade.status === "proof_submitted" && (
            <div className="space-y-2">
              {trade.paymentProofUrl && (
                <div>
                  <p className="text-xs text-black font-bold mb-1.5">Payment proof from buyer:</p>
                  <div className="relative border border-neutral-200 overflow-hidden cursor-zoom-in group"
                    onClick={() => setZoom(true)}>
                    <img src={`${BASE}${trade.paymentProofUrl}`} alt="Payment proof"
                      className="w-full object-contain max-h-40"/>
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 flex items-center justify-center transition-colors">
                      <ZoomIn size={20} className="text-black opacity-0 group-hover:opacity-100 transition-opacity"/>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                ...
                <button onClick={dispute}
                  className="flex items-center justify-center gap-1 border border-red-900 text-red-400 text-xs font-bold py-2 hover:bg-red-900/20">
          ...
          {isSeller && trade.status === "pending" && (
            <div className="bg-neutral-50 border border-neutral-200 px-3 py-2">
              <p className="text-xs text-neutral-500">Waiting for @{trade.buyer.username} to send {trade.assetAmount} and upload payment proof.</p>
            </div>
          )}

          {/* Seller's Lightning address hint */}
          {isBuyer && trade.seller.lightningAddress && (
            <div className="bg-neutral-50 border border-neutral-200 px-3 py-2 mt-2">
              <p className="text-xs text-neutral-600 mb-1">After seller confirms, BTC arrives at your Lightning address:</p>
              <code className="text-xs text-black font-mono">{trade.buyer.lightningAddress}</code>
            </div>
          )}
        </>)}
        ...
        {trade.status === "released" && (
          <div className="flex items-center gap-2 bg-green-900/20 border border-green-900/40 px-3 py-2">
            ...
            <span className="text-xs text-green-400 font-bold">Trade complete!</span>
          </div>
        )}
        ...
        {trade.status === "disputed" && (
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 bg-red-900/20 border border-red-900/40 px-3 py-2">
              ...
              <span className="text-xs text-red-400 font-bold">Dispute open — admin reviewing.</span>
            </div>
            {trade.paymentProofUrl && (
        ...
        {!["released","disputed","refunded"].includes(trade.status) && !isFiat && trade.status !== "pending" && (
          <button onClick={dispute} className="w-full text-xs text-neutral-500 hover:text-red-400 mt-2 py-1">
            Open Dispute
          </button>
        )}

        <p className="text-xs text-neutral-400 mt-3 text-center">VBC Escrow • Sats locked until confirmed</p>
      </div>
      ...
      {zoom && trade.paymentProofUrl && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4" onClick={() => setZoom(false)}>
          <button onClick={() => setZoom(false)} className="absolute top-4 right-4 text-black hover:text-black">
            <X size={24}/>
          </button>
          <img src={`${BASE}${trade.paymentProofUrl}`} alt="Payment proof"
